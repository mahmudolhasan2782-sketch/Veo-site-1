...
export const refinePrompt = async (originalPrompt: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      Your task is to rewrite the following user prompt to be optimized for video generation. 
      
      Rules:
      1. If the prompt is not in English, translate it to English first.
      2. Describe lighting, camera movement, and atmosphere concisely.
      3. Keep it under 100 words.
      4. Return ONLY the refined prompt text, no explanations.
