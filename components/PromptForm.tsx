...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.FRAMES_TO_VIDEO]:
      'Describe the action, lighting, or movement...',
    [GenerationMode.REFERENCES_TO_VIDEO]:
  ...
  switch (generationMode) {
    ...
    case GenerationMode.FRAMES_TO_VIDEO:
      ...
      if (isSubmitDisabled) {
        tooltipText = 'A start image is required.';
      }
      break;
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          <div className="relative" ref={modeSelectorRef}>
            <button
              ...
              <span className="font-medium text-sm whitespace-nowrap">
                {modeDisplayNames[generationMode] || generationMode}
              </span>
            </button>
            {isModeSelectorOpen && (
              <div className="absolute bottom-full mb-2 w-60 bg-[#2c2c2e] border border-gray-600 rounded-lg shadow-xl overflow-hidden z-10">
                {selectableModes.map((mode) => (
                  <button
                    ...
                    {modeIcons[mode]}
                    <span>{modeDisplayNames[mode] || mode}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex-grow flex items-center gap-2">
            <textarea
              ...
              className="w-full bg-transparent focus:outline-none resize-none text-base text-gray-200 placeholder-gray-500 max-h-48 py-2 min-h-[44px]"
              rows={1}
            />
            {prompt.trim() && (
              <button
                ...
                disabled={isRefining}
                className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-indigo-400 hover:text-indigo-300 hover:bg-indigo-400/10 rounded-lg transition-colors"
                title="Refine and Translate Prompt">
                {isRefining ? (
